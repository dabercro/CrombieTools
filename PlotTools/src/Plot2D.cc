               ...
               Int_t NumYBins, Double_t MinY, Double_t MaxY)
{
  ...
  if (fFunctionString == "") {
    ...
    std::cerr << "You haven't set a function!" << std::endl;
    exit(256);

  }
  if (fInExprX == "" && fInExprXs.size() == 0) {

    std::cerr << "You haven't initialized an x expression yet!" << std::endl;
    exit(257);
  ...
  if(NumPlots == 0){
    ...
    std::cerr << "Nothing has been initialized in resolution plot." << std::endl;
    exit(300);
