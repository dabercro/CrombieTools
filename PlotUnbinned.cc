...
#include "PlotUnbinned.h"

ClassImp(PlotUnbinned)

//--------------------------------------------------------------------
PlotUnbinned::PlotUnbinned() :
  fInExprX(""),
  fDumpingFits(false),
  fNumFitDumps(0)
{
...
//--------------------------------------------------------------------
void
PlotUnbinned::SetParameterLimits(Int_t param, Double_t low, Double_t high)
{
...
PlotUnbinned::GetRatioToLines(std::vector<TGraphErrors*> InGraphs, std::vector<TGraphErrors*> RatioGraphs)
{
...
//--------------------------------------------------------------------
std::vector<TGraphErrors*>
PlotUnbinned::GetRatioToLine(std::vector<TGraphErrors*> InGraphs, TGraphErrors *RatioGraph)
{
...
//--------------------------------------------------------------------
std::vector<TGraphErrors*>
PlotUnbinned::GetRatioToPoint(std::vector<TGraphErrors*> InGraphs, Double_t RatioPoint, Double_t PointError)
{
...
//--------------------------------------------------------------------
void
PlotUnbinned::MakeFitGraphs(Int_t NumXBins, Double_t *XBins,
                            Int_t NumYBins, Double_t MinY, Double_t MaxY)
{
  ...
  Double_t params[13] = {0,0,40,0.01,0,30,0.01,0,60,0.01,0,1000,0};
  TF2 *fitA = new TF2("fittums",fitFunc,-150,150,10,1000,11);
  ...
  TF2 *fitB = new TF2("fitto",testFunc,-150,150,0,1000,5);
  Double_t paramsB[5] = {0,0,30,20,40};
  fitB->SetParameters(paramsB);
  fitB->SetLineColor(kBlue);

  hist->Fit(fitB,"MLE");

  fitA->SetParameter(0,fitB->GetParameter(0));
  fitA->SetParameter(1,fitB->GetParameter(1));
  fitA->SetParameter(2,fitB->GetParameter(2));
  fitA->SetParameter(5,fitB->GetParameter(3));
  fitA->SetParameter(8,fitB->GetParameter(4));
...
//--------------------------------------------------------------------
void
PlotUnbinned::MakeFitGraphs(Int_t NumXBins, Double_t MinX, Double_t MaxX,
                            Int_t NumYBins, Double_t MinY, Double_t MaxY)
{
  ...
  MakeFitGraphs(NumXBins,XBins,NumYBins,MinY,MaxY,Channel,isMC);
}

//--------------------------------------------------------------------
TCanvas*
PlotUnbinned::MakeCanvas(std::vector<TGraphErrors*> theGraphs,
...
//--------------------------------------------------------------------
void
PlotUnbinned::MakeCanvas(TString FileBase, std::vector<TGraphErrors*> theGraphs,
                           TString CanvasTitle, TString XLabel, TString YLabel,
                           Double_t YMin, Double_t YMax, Bool_t logY)
{
